...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h2 class="text-2xl font-semibold text-black dark:text-white">Riwayat Transaksi</h2>
    </div>
</div>

<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white pt-4 dark:border-white/[0.05] dark:bg-white/[0.03]">
    {{-- Search / Filter by Date --}}
    <div class="px-6 mb-4" x-data="{
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center justify-between w-full">
            {{-- search --}}
            <div class="relative flex-1 max-w-[350px]">
                ...
                <input type="text" x-model="search" @input.debounce.500ms="performSearch()" placeholder="Cari jenis transaksi atau nominal..." class="h-[42px] w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pl-[42px] pr-4 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 xl:w-[350px]"/>
            </div>
            ...
            <table class="w-full">
                <thead class="border-y border-gray-100 bg-gray-50 dark:border-white/[0.05] dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-400">Saldo Akhir</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($transaksis as $i => $transaksi)
                        <tr class="border-b border-gray-100 dark:border-white/[0.05] hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-4 sm:px-6 py-3.5">
                                <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ $transaksis->firstItem() + $i }}</span>
                            </td>
                            <td class="px-4 sm:px-6 py-3.5">
                                <span class="block text-theme-sm text-gray-700 dark:text-gray-400">{{ \Carbon\Carbon::parse($transaksi->created_at)->translatedFormat('d M Y, H:i') }}</span>
                            </td>
                            <td class="px-4 sm:px-6 py-3.5">
                            ...
                            <td class="px-4 sm:px-6 py-3.5">
                                <span class="block text-theme-sm font-semibold {{ $transaksi->jenis_transaksi === 'setor' ? 'text-green-600 dark:text-green-500' : 'text-orange-600 dark:text-orange-500' }}">
                                    {{ $transaksi->jenis_transaksi === 'setor' ? '+' : '-' }} Rp {{ number_format($transaksi->nominal, 0, ',', '.') }}
                                </span>
                            </td>
                            <td class="px-4 sm:px-6 py-3.5">
                                <span class="block text-theme-sm text-gray-700 dark:text-gray-400 font-medium">Rp {{ number_format($transaksi->saldo_sesudah, 0, ',', '.') }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Anda belum melakukan penyetoran atau penarikan.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
