...
@php
    use App\Helpers\NasabahMenuHelper;
    $menuGroups = NasabahMenuHelper::getMenuGroups();
...
<aside id="sidebar"
    ...
    <!-- Logo Section -->
    <div class="pt-8 pb-7 xl:pt-1 xl:pb-1 flex"
        :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ?
        'xl:justify-center' :
        'justify-start'">
        <a href="/" class="hidden xl:block">
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                class="dark:hidden w-[200px] h-auto" src="/images/logo/logo.png" alt="Logo" />
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                class="hidden dark:block w-[200px] h-auto" src="/images/logo/logo.png" alt="Logo" />
            <img x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
    ...
    <div class="flex flex-col overflow-y-auto duration-300 ease-linear no-scrollbar">
                                    ...
                                    @if (isset($item['subItems']))
                                        ...
                                        <button @click="toggleSubmenu({{ $groupIndex }}, {{ $itemIndex }})"
                                            ...
                                            <span :class="isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) ?
                                                ...
                                                {!! NasabahMenuHelper::getIconSvg($item['icon']) !!}
                                            </span>
        ...
        <div x-data x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen" x-transition class="mt-auto">
            @include('layouts.nasabah.sidebar-widget')
        </div>

    </div>

    <!-- Logout Button -->
    <div class="mt-auto px-4 pb-5 pt-3 border-t border-gray-200 dark:border-gray-800">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="flex items-center gap-3 px-3 py-2 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-red-50 hover:text-red-600 dark:text-gray-400 dark:hover:bg-red-500/10 dark:hover:text-red-500 w-full justify-start xl:justify-center transition-colors"
                :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ? 'xl:justify-center' : 'justify-start'">
                <span class="text-gray-500 group-hover:text-red-600 dark:group-hover:text-red-500 transition-colors">
