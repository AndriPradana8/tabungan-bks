...
@section('content')
  ...
  @if (session('error'))
    <div x-data="{ show: true }" 
      ...
      <div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-[1.5px] border-red-500 text-red-500 mt-0.5">
        <i class="fa-solid fa-exclamation text-[12px] font-bold"></i>
      </div>
      <div class="w-full">
  ...
  <div x-data="{
      ...
      selectedNasabahData: {},
      selectNasabah(id, data) {
          if (this.selectedNasabahId === id) {
              this.selectedNasabahId = null;
              this.selectedNasabahData = {};
          } else {
              this.selectedNasabahId = id;
              this.selectedNasabahData = data;
          }
      }
  }">
            ...
            <div class="flex items-center justify-between gap-4 w-full">
                ...
                <form @submit.prevent>
                    <div class="relative" x-data="{
                        ...
                        <input type="text" x-model="search" @input.debounce.500ms="performSearch()" placeholder="Cari nasabah..." class="h-[42px] w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pl-[42px] pr-4 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-blue-800 xl:w-[300px]"/>
                    </div>
                </form>

                <div class="flex gap-2">
                    <button 
                        @click="if(selectedNasabahId) $dispatch('open-modal-setor', selectedNasabahData)"
                        :disabled="!selectedNasabahId"
                        :class="!selectedNasabahId ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-600'"
                        class="inline-flex items-center gap-2 rounded-lg bg-green-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs dark:bg-green-600 transition-colors">
                        <i class="fa-solid fa-arrow-down"></i>
                        Setor
                    </button>
                    
                    <button 
                        @click="if(selectedNasabahId) $dispatch('open-modal-tarik', selectedNasabahData)"
                        :disabled="!selectedNasabahId"
                        :class="!selectedNasabahId ? 'opacity-50 cursor-not-allowed' : 'hover:bg-orange-600'"
                        class="inline-flex items-center gap-2 rounded-lg bg-orange-500 px-4 py-2.5 text-theme-sm font-medium text-white shadow-theme-xs dark:bg-orange-600 transition-colors">
        ...
        <div id="data-container">
            ...
            <table class="w-full">
                <thead class="px-6 py-3.5 border-t border-gray-100 border-y bg-gray-50 dark:border-white/[0.05] dark:bg-gray-900">
                    <tr>
                        <th class="px-4 py-3 sm:px-6 w-10"></th>
                        ...
                        <th class="px-6 py-3 font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Saldo</th>
                ...
                <tbody>
                    @forelse ($nasabahs as $index => $user)
                        <tr class="border-b border-gray-100 dark:border-white/[0.05] transition-colors"
                            :class="selectedNasabahId === {{ $user->id }} ? 'bg-brand-50/50 dark:bg-brand-500/10' : 'hover:bg-gray-50 dark:hover:bg-white/[0.02]'">
                            <td class="px-4 py-3 sm:px-6">
                                <div class="flex items-center">
                                    <input type="checkbox" 
                                        :checked="selectedNasabahId === {{ $user->id }}"
                                        @click="selectNasabah({{ $user->id }}, {
                                            id: {{ $user->id }},
                                            nama: '{{ addslashes($user->nama) }}',
                                            nik: '{{ addslashes($user->nik) }}',
                                            alamat: '{{ addslashes($user->nasabah->alamat ?? '-') }}',
                                            saldo: {{ $user->tabungan ? $user->tabungan->saldo : 0 }}
                                        })"
                                        class="w-4 h-4 text-brand-500 border-gray-300 rounded focus:ring-brand-500 dark:focus:ring-brand-600 dark:ring-offset-gray-900 dark:border-gray-700 cursor-pointer">
                                </div>
                            </td>
                            ...
                            <td class="px-4 sm:px-6 py-3.5">
                                <p class="text-gray-700 text-theme-sm dark:text-gray-400 font-medium">Rp {{ number_format($user->tabungan ? $user->tabungan->saldo : 0, 0, ',', '.') }}</p>
                            </td>
                            <td class="px-4 sm:px-6 py-3.5">
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('admin.tabungan.riwayat', $user->id) }}" class="inline-flex items-center gap-1.5 text-brand-500 hover:text-brand-600 text-theme-sm font-medium transition-colors">
  ...
  @include('admin.modal.modal-setor-tabungan')
  @include('admin.modal.modal-tarik-tabungan')
