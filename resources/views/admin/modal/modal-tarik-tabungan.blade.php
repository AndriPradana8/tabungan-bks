<div x-data="{
    ...
    @open-modal-tarik.window="nasabah = $event.detail; nominal = ''; formattedNominal = '';">
    
    <x-ui.modal @open-modal-tarik.window="open = true" @close-modal-tarik.window="open = false" class="max-w-[500px] p-6 lg:p-8">
        <div class="flex flex-col overflow-y-auto custom-scrollbar">
            ...
            <form action="{{ route('admin.tabungan.tarik') }}" method="POST">
                @csrf
                ...
                <div class="mb-5 space-y-4">
                    <div class="rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                        <h4 class="mb-3 text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Informasi Nasabah</h4>
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Nominal Tarik (Rp) <span class="text-red-500">*</span>
                        </label>
                        ...
                        <p x-show="nominal > 0 && nominal < 1000" class="mt-2 text-sm text-red-500">
                            * Minimal tarik Rp 1.000
                        </p>
                        <p x-show="nominal > nasabah.saldo" class="mt-2 text-sm text-red-500">
                            * Peringatan: Nominal penarikan melebihi saldo.
                        </p>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 mt-8">
                    ...
                    <button type="submit"
                        :disabled="!nominal || nominal < 1000 || nominal > nasabah.saldo"
                        :class="!nominal || nominal < 1000 || nominal > nasabah.saldo ? 'opacity-50 cursor-not-allowed' : 'hover:bg-orange-600'"
                        class="rounded-lg bg-orange-500 px-5 py-2.5 text-sm font-medium text-white transition-colors">
                        Proses Tarik
                    </button>
                </div>
            </form>
