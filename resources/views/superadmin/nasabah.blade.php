  ...
  <div x-data>
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white pt-4 dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-4 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <!-- The 'Tambah Nasabah' button has been removed -->
        </div>
                        ...
                        <tr class="border-b border-gray-100 dark:border-white/[0.05]">
                            ...
                            <td class="px-4 sm:px-6 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    })" class="hover:text-brand-500 text-gray-500 transition-colors" title="Lihat Detail">
                                        <i class="fa-solid fa-eye text-lg"></i>
                                    </button>
                                    
                                    <button @click="$dispatch('open-modal-toggle-nasabah', { id: {{ $user->id }}, nama: '{{ addslashes($user->nama) }}', status_akun: '{{ addslashes($user->status_akun) }}' })"
                                        class="hover:text-{{ $user->status_akun === 'aktif' ? 'orange' : 'green' }}-500 text-gray-500 transition-colors"
                                        title="{{ $user->status_akun === 'aktif' ? 'Nonaktifkan Nasabah' : 'Aktifkan Nasabah' }}">
                                        <i class="fa-solid fa-toggle-{{ $user->status_akun === 'aktif' ? 'on' : 'off' }} text-lg"></i>
                                    </button>

                                    <button @click="$dispatch('open-modal-delete-nasabah', { id: {{ $user->id }}, nama: '{{ addslashes($user->nama) }}' })"
                                        class="hover:text-red-500 text-gray-500 transition-colors" title="Hapus Nasabah">
...
@include('superadmin.modal.modal-detail-nasabah')
@include('superadmin.modal.modal-toggle-nasabah')
@include('superadmin.modal.modal-delete-nasabah')
