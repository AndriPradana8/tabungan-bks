<!-- Modal Toggle Nasabah -->
<x-ui.modal @close-modal-toggle-nasabah.window="open = false" :isOpen="false" :blurBackdrop="false" :showCloseButton="false" class="max-w-[400px] p-6 text-center">
    <div x-data="{ toggleData: {} }" @open-modal-toggle-nasabah.window="open = true; toggleData = $event.detail">
        <h4 class="text-xl font-bold text-gray-800 dark:text-white mb-2" x-text="toggleData.status_akun === 'aktif' ? 'Nonaktifkan Nasabah?' : 'Aktifkan Nasabah?'"></h4>
        ...
        <div class="flex justify-center gap-3">
            ...
            <form :action="`/superadmin/nasabah/${toggleData.id}/toggle`" method="POST" class="m-0 p-0">
                @csrf
                @method('PUT')
